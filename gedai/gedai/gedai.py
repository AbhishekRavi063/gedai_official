...
class Gedai:
    ...
    def fit_epochs(
        ...
        for w, (fmin, fmax) in enumerate(freq_bands):
            ...
            for e, wavelet_epoch_data in enumerate(wavelet_epochs_data):
                covariance = np.atleast_2d(np.cov(wavelet_epoch_data))
            ...
            # Professor's fix: no hard bypass, just extended bounds above.
            wavelet_fit["bypass"] = False
            wavelet_fit["ignore"] = False
    ...
    def transform_epochs(
        ...
        for wavelet_fit in self.wavelets_fits:
            ...
            ignore = wavelet_fit.get("ignore", False)
            bypass = wavelet_fit.get("bypass", False)

            if ignore:
                # Zero out this band entirely
                cleaned_epochs_wavelet[:, :, band_idx, :] = 0
            elif bypass:
                # Leave this brain-frequency band completely untouched 
                # (Alpha/Beta waves survive 100%)
                pass
            else:
    ...
    def transform_raw(
        ...
        raw_data = raw.get_data(verbose=False).astype(np.float32)
        n_channels, n_times = raw_data.shape

        window_size = int(raw.info["sfreq"] * duration)
        window = create_cosine_weights(window_size).astype(np.float32)

        raw_corrected = np.zeros_like(raw_data, dtype=np.float32)
        weight_sum = np.zeros_like(raw_data, dtype=np.float32)
        ...
        for i in range(0, len(starts), batch_size):
            ...
            all_segments_array = np.array(all_segments, dtype=np.float32)
            ...
            corrected_segments = corrected_segments_epochs.get_data(verbose=False).astype(np.float32)

            # Apply windowing and reconstruct
            for s, start in enumerate(batch_starts):
