...
def _sensai_to_eigen(sensai_value, eigenvalues):
    ...
    log_eig_val_all = np.log(all_diagonals[all_diagonals > 1e-10]) + 100
    if len(log_eig_val_all) == 0:
        return 1e-10
...
def _eigen_to_sensai(eigenvalue, eigenvalues):
    ...
    if len(log_eig_val_all) == 0:
        return 105.0
    threshold1 = np.log(max(eigenvalue, 1e-10)) + 100
...
def _sensai_score(epochs, threshold, reference_cov, n_pc, noise_multiplier):
    ...
    for e, (epoch_clean_data, epoch_artefact_data) in enumerate(
        ...
        epoch_clean_covariance = np.atleast_2d(np.cov(epoch_clean_data))
        ...
        epoch_artefact_covariance = np.atleast_2d(np.cov(epoch_artefact_data))
